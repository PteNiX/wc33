...
for (let i=0; i < nwc3lTotal.length; i++) { 
    ...
    allEasy.push(parseInt(nwc3lTotalEasy[i],10)+parseInt(nwc3lTotalEasyM[i],10)); 
...
for (let i=0; i < nwc3lWinMap.length; i++) { 
    ...
    sumNwc3lMaps.push(parseInt(nwc3lWinMapEasy[i],10)+parseInt(nwc3lLoseMapEasy[i],10)); 
...
for (let k=0; k < allEasy.length; k++) { 
    if (sumNwc3lMaps[k]!=0){
    document.querySelectorAll('.nwc3l-winrate')[k].innerHTML = (Math.round((nwc3lWinMapEasy[k]/sumNwc3lMaps[k])*100)+"%");}
...
for (let i=0; i < masterWinMap.length; i++) { 
    ...
    sumMasterMaps.push(parseInt(masterWinMapEasy[i],10)+parseInt(masterLoseMapEasy[i],10)); 
...
for (let k=0; k < allEasy.length; k++) { 
    ...
    document.querySelectorAll('.master-winrate')[k].innerHTML = (Math.round((masterWinMapEasy[k]/sumMasterMaps[k])*100)+"%");}
...
for (let i=0; i < masterWinMap.length; i++) { 
    ...
    totalWinMap.push(parseInt(masterWinMapEasy[i],10)+parseInt(nwc3lWinMapEasy[i],10)); 
    totalLoseMap.push(parseInt(masterLoseMapEasy[i],10)+parseInt(nwc3lLoseMapEasy[i],10)); 
...
for (let k=0; k < totalWinMap.length; k++) { 
    if (totalMap[k]!=0){
    document.querySelectorAll('.total-winrate')[k].innerHTML = (Math.round((totalWinMap[k]/totalMap[k])*100)+"%");}
...
if (document.getElementById('btn-close').value==="p2n") {
    ...
    document.getElementById('content').innerHTML= "<pre>wrong password,\ntry again </pre>";
